...
def getContours(img):
    ...
    for cnt in contours:
        ...
        if area>600: #600, #1300
            ...
            #elif objCor>6 and objCor<9: objectType= "Hexagon"
...
while (cap.isOpened()):
    ...
    try:
        ...
        h = int(dimensions[0])
        w = int(dimensions[1])
        img = img[int(h/4.8): int(h/2.4) , int(w/2.4):int(w/1.1) ]#int(h/6): int(h/2) , int(w/2.5):w (for avi), 0: int(h/2) , int(w/2):w (for mp4)
        print(img.shape)
        original_img = img
        imgContour = img.copy()
        ...
        x,y,w,h,shape,area,sides= getContours(imgCanny)
        ...
        lower = np.array([80, 141, 65]) # for pov
        ...
        if number_of_black_pix < blue_imgResult.size-800:
            blue_color = True

        #------YELLOW MASK--------
        #lower = np.array([21,65,127]) #16,89,100]
        #upper = np.array([66,99,169]) #[55,246,181]
        lower = np.array([20,89,75]) #[9,71,140]
        ...
        upper = np.array([100,255,170]) #[71,163,250]
        ...
        if(shape=="Hexagon" and red_color==True):
            ...
            cv2.putText(imgContour, "Crosswalk",
                        (x + (w // 2) + 10, y + (h // 2) - 70), cv2.FONT_HERSHEY_COMPLEX, 0.7,
                        (0, 0, 0), 2)
        elif(shape=="Hexagon" and blue_color==True and sides>14):
            print("parking")
            ...
            cv2.putText(imgContour, "roundabout",
                        (x + (w // 2) + 10, y + (h // 2) -70), cv2.FONT_HERSHEY_COMPLEX, 0.7,
                        (0, 0, 0), 2)
        elif (shape == "Rectangle" and green_color == True and red_line==False):
            print("highway_entry")
            cv2.putText(imgContour, "highway_entry",
                        (x + (w // 2) + 10, y + (h // 2) -70), cv2.FONT_HERSHEY_COMPLEX, 0.7,
                        (0, 0, 0), 2)
        
        elif (shape == "Rectangle" and green_color == True and red_line == True):
            print("highway_exit")
        ...
        elif (shape == "Arrow" and blue_color == True and sides<10):
            print("one way")
        ...
        imgStack = stackImages(0.8,([img_cropped,imgHSV],[red_mask,red_imgResult],[blue_mask,blue_imgResult],[yellow_mask,yellow_imgResult],[green_mask,green_imgResult]))
        #imgStack = stackImages(0.7,([img,imgGray,imgBlur], [imgCanny,imgContour,imgBlank]))
