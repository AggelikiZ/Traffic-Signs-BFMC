...
#------------------SHAPE DETECTION-----------------------------------#
path = 'output_video1706712013.179189.mp4'
...
while (cap.isOpened()):
    ...
    try:
        ...
        h = int(dimensions[0])
        w = int(dimensions[1])
        img = img[int(h/6): int(h/2.5) , int(w/2.4):int(w/1) ]#int(h/6): int(h/2) , int(w/2.5):w (for avi), 0: int(h/2) , int(w/2):w (for mp4)
        original_img = img
        imgContour = img.copy()
        ...
        #detect color
        red_color = False
        number_of_black_pix_r= np.sum(red_imgResult == 0)
        if number_of_black_pix_r<red_imgResult.size-800:
            red_color = True
            print("red :", number_of_black_pix_r)

        # ------BLUE MASK--------
        #lower = np.array([105, 176, 33]) #[109, 205, 22]
        lower = np.array([70, 141, 10]) # for pov
        ...
        number_of_black_pix_b = np.sum(blue_imgResult == 0)
        if number_of_black_pix_b < blue_imgResult.size-800:
            blue_color = True
            print("blue :", number_of_black_pix_b)

        #------YELLOW MASK--------
        #lower = np.array([21,65,127]) #16,89,100]
        #upper = np.array([66,99,169]) #[55,246,181]
        lower = np.array([5,89,75]) #[9,71,140]
        ...
        #detect color
        yellow_color = False
        number_of_black_pix_y = np.sum(yellow_imgResult == 0)
        #print("black",number_of_black_pix)
        if number_of_black_pix_y<yellow_imgResult.size - 800:
            yellow_color = True
            print("yellow :", number_of_black_pix_y)
        ...
        if(shape=="Hexagon" and red_color==True):
            ...
            cv2.putText(imgContour, "prioriy",
                        (x + (w // 2) + 10, y + (h // 2) -70), cv2.FONT_HERSHEY_COMPLEX, 0.7,
                        (0, 0, 0), 2)
        elif (shape == "Circles" and blue_color == True):
            print("roundabout")
