...
while (cap.isOpened()):
    ...
    try:
        ...
        img = img[int(h/4.3): int(h/2.4) , int(w/2.2):int(w/1) ]
        ...
        lower = np.array([125,90,45]) #[124,90,75] , [0,79,97]
        ...
        lower = np.array([105, 110, 35]) # for pov
        ...
        print(number_of_black_pix,blue_imgResult.size)
        ...
        #------GREEN MASK--------
        lower = np.array([45,80,30]) #[9,71,140]
        upper = np.array([94,255,145]) #[71,163,250]
        ...
        if((shape=="Hexagon" or shape == "Arrow" )and  red_color==True):
            print("Stop")
            sign = "Stop"
            Found = True
            ...
            cv2.putText(imgContour, "Crosswalk",
                        (x + (w // 2) + 10, y + (h // 2) - 70), cv2.FONT_HERSHEY_COMPLEX, 0.7,
                        (0, 0, 0), 2)
        elif((shape=="Hexagon" or shape == "Arrow" ) and (blue_color == True) and (sides>10)):
            print("sides",sides)
            print("Parking")
            sign = "Parking"
            Found = True
